"""Build self-contained HTML match reports.
...
Plotly is optional. The Movement tab is omitted when Plotly is unavailable,
disabled, or no map image is provided.
"""
...
class ReportOptions:
    ...
    Attributes:
        title: Browser title used for the generated HTML document.
        include_movement: Whether to attempt the optional Plotly movement tab.
    """
...
def _build_movement_tab(match: ParsedMatch, map_b64: str | None) -> str:
...
def build_html_report(
    ...
    *,
    ...
    Args:
        ...
        assets: Optional local map/icon assets used to enrich the report.
        options: Rendering options.
        map_b64: Pre-encoded map image for callers that already loaded one.
    ...
    movement_html = _build_movement_tab(match, map_b64) if options.include_movement else ""
    if movement_html:
...
def build_html(match: ParsedMatch, map_b64: str | None = None) -> str:
    """Backward-compatible alias for the previous example helper name."""
    return build_html_report(match, map_b64=map_b64)
...
def write_html_report(
    ...
    *,
    ...
    path.write_text(
        build_html_report(match, assets=assets, options=options, map_b64=map_b64),
        encoding="utf-8",
    )
    return path
