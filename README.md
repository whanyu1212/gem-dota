...
- **Roshan drops** — the Aegis, Cheese, Refresher Shard, and Roshan's Banner pickups are not in the combat log, so they are recovered from the entity stream instead (`RoshanKill.drops`). This is reliable for the items Roshan currently drops; it is not an open-ended pickup feed, so a future drop type would need its entity class added to the snapshot map.
...
- **Truncated/live replays** — incomplete replays raise by default when the stream ends early. Pass `allow_partial=True` to `gem.parse()` (and `parse_to_json` / `parse_to_dataframe` / `parse_to_parquet`) to recover best-effort partial output; the result exposes `parse_error` and `truncated_at_tick` so callers can detect incompleteness.
